...
ws.on("message", (data) => {
    ...
    if (message.charAt(0) === "#") {
        ...
        serverKey = data.toString().substring(1)
        // send the client's public key
        ws.send("#" + publicKeyClient)
    } else {
        // decrypt the message sent from server
...
for await (const line of createInterface({ input: process.stdin })) {
    // encrypt client's message with server public key
    const encryptedMessage = crypto.publicEncrypt(serverKey.toString(), line)
